...
async def breath_search(
    ...
    with use_read_identity(cabinet):
        return await _with_notice(
            ...
            op="breath_search",
            args={"query": query, "domain": domain, "max_results": max_results, "cabinet": cabinet},
        )
...
@mcp.tool()
async def dream(window_hours: Optional[int] = 48, cabinet: Optional[str] = "") -> str:
    ...
    with use_read_identity(cabinet):
        return await _with_notice(
            ...
            op="dream",
            args={"window_hours": window_hours, "cabinet": cabinet},
        )
...
async def media_read(
    ...
    """只读获取 Ombre Brain 中已持久化的媒体原文件。且只传一种定位方式：bucket_id=返回该桶关联的全部媒体；media_path=返回库内 `_media/...` 指定文件。工具返回 5 分钟有效的标准 MCP resource_link，不返回 base64。路径穿越、绝对路径、符号链接和 `_media/` 之外的文件都会被拒绝。"""
    return await _web.media.tool_result(
